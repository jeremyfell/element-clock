...
function update(repeat) {
	...
	if (display) {
		if (!sfull) {
			...
			sp.innerHTML = element_symbol[95 - 34 * Number(post)];
			np.innerHTML = element_name[95 - 34 * Number(post)];
			mp.innerHTML = element_mass[95 - 34 * Number(post)];

		} else {
			ep.style.visibility = "hidden";
		}
		sh.innerHTML = element_symbol[h];
		sm.innerHTML = element_symbol[m];
		ss.innerHTML = element_symbol[s];
		ah.innerHTML = h;
		am.innerHTML = m;
		as.innerHTML = s;

		if (sname) {
			nh.innerHTML = element_name[h];
			nm.innerHTML = element_name[m];
			ns.innerHTML = element_name[s];
		...
		if (smass) {
			mh.innerHTML = element_mass[h];
			mm.innerHTML = element_mass[m];
			ms.innerHTML = element_mass[s];
		...
		if (sgroup) {
			groupUpdate(element_group[h], eh);
			groupUpdate(element_group[m], em);
			groupUpdate(element_group[s], es);
			groupUpdate(2, ep);
		} else {
			groupDefault(eh);
			groupDefault(em);
...
for (var n = 1; n <= 118; n++) {
	...
	celement.childNodes[1].innerHTML = element_symbol[n];
	celement.onclick = function() {window.open(wiki + element_name[Number(this.childNodes[0].innerHTML)])};
	celement.title = element_name[n];
